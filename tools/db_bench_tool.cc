...
#include <fstream> // timestamp throughput
...
DEFINE_string(history_file_name, "", "MixGraph ops/sec trace file name");
...
DEFINE_bool(
		cache_index_and_filter_blocks_for_mmap_read, true,
		"Cache mmaped index and filter blocks in table reader | block cache");

DEFINE_bool(cache_data_blocks_for_mmap_read, false,
		"Cache mmaped data blocks in block cache");
...
DEFINE_bool(recycle_dcpmm_sst, true,
		"Recycle obsoleted sst to avoid page-fault and page-zeroing "
		"overhead on pmem");
...
DEFINE_bool(dcpmm_wal_enable, true,
		"Store WAL file to dcpmm device and use pmdk to write/read it. ");

DEFINE_bool(dcpmm_kvs_enable, true,
		"If separately store values with libpmemobj");

DEFINE_string(dcpmm_kvs_mmapped_file_fullpath, "/pmem/zzunny/pmem-rocksdb/pools/pool",
...
DEFINE_int64(dcpmm_kvs_value_thres, 64, "Use kvs for values larger than it. ");

DEFINE_bool(dcpmm_compress_value, false,
...
namespace ROCKSDB_NAMESPACE {
	...
	class Benchmark {
		private:
			/* timestamp */
			std::vector<std::map<uint64_t, uint64_t>> get_history_;
			std::vector<std::map<uint64_t, uint64_t>> put_history_;
			std::vector<std::map<uint64_t, uint64_t>> seek_history_;
...
#endif  // !ROCKSDB_LITE
					{
						/* timestamp vector allocation */
						fprintf(stderr, "vector allocation start\n");
						get_history_.resize(FLAGS_threads);
						put_history_.resize(FLAGS_threads);
						seek_history_.resize(FLAGS_threads);
						fprintf(stderr, "vector allocation end\n");
		...
		private:
			...
			void WriteHistoryFile(
					std::vector<std::map<uint64_t, uint64_t>> put,
					std::vector<std::map<uint64_t, uint64_t>> get,
					std::vector<std::map<uint64_t, uint64_t>> seek,
					std::string filename) {
				std::ofstream stream(filename); //To Write into a File, Use "ofstream"
				stream << "#SEC\tGET\tPUT\tSEEK\n";
					// search last sec
					uint64_t last_sec = 0;
					for(auto& cur_map : put) {
						if (!cur_map.empty()) last_sec = std::max(last_sec, cur_map.rbegin()->first);
					}
					for(auto& cur_map : get) {
						if (!cur_map.empty()) last_sec = std::max(last_sec, cur_map.rbegin()->first);
					}
					for(auto& cur_map : seek) {
						if (!cur_map.empty()) last_sec = std::max(last_sec, cur_map.rbegin()->first);
					}
					fprintf(stdout, "last sec : %ld\n", last_sec);

					// sum up until last sec
					for(size_t i=0; i<=last_sec; i++) {
						uint64_t total_get= 0;
						uint64_t total_put = 0;
						uint64_t total_seek = 0;
						for(auto& cur_map : get) {
							auto iter = cur_map.find(i);
							if(iter != cur_map.end())
								total_get += iter->second;
						}
						for(auto& cur_map : put) {
							auto iter = cur_map.find(i);
							if(iter != cur_map.end())
								total_put += iter->second;
						}
						for(auto& cur_map : seek) {
							auto iter = cur_map.find(i);
							if(iter != cur_map.end())
								total_seek += iter->second;
						}
						stream << i << "\t" << total_get << "\t" << total_put << "\t" << total_seek << "\n";
					}
			...
			Stats RunBenchmark(int n, Slice name,
				...
				// History dump to file				
				/*
				for(auto& cur_map : get_history_) {
					for(auto& kv : cur_map) {
						fprintf(stderr, "%ld %ld\n", kv.first, kv.second);
					}
				}
				*/
				fprintf(stderr, "history_file_name : %s\n", FLAGS_history_file_name.c_str());
				WriteHistoryFile(
						get_history_, 
						put_history_,
						seek_history_,
						FLAGS_history_file_name);
				//WriteHistoryFile(put_history_, "/home/zzunny/KVstores/pmem-rocksdb/scripts/all_random/put_log");
				//WriteHistoryFile(seek_history_, "/home/zzunny/KVstores/pmem-rocksdb/scripts/all_random/seek_log");
			...
			void InitializeOptionsFromFlags(Options* opts) {
				...
				options.recycle_dcpmm_sst = FLAGS_recycle_dcpmm_sst;
				...
				if (FLAGS_use_plain_table) {
					...
					block_based_options.cache_index_and_filter_blocks_for_mmap_read =
						FLAGS_cache_index_and_filter_blocks_for_mmap_read;
					block_based_options.cache_data_blocks_for_mmap_read =
						FLAGS_cache_data_blocks_for_mmap_read;
...
#ifdef ON_DCPMM
				options.dcpmm_kvs_enable = FLAGS_dcpmm_kvs_enable;
				options.dcpmm_kvs_mmapped_file_fullpath =
					FLAGS_dcpmm_kvs_mmapped_file_fullpath;
			...
			void DoWrite(ThreadState* thread, WriteMode write_mode) {
				const int test_duration =
					write_mode == (RANDOM && !fill) ? FLAGS_duration : 0;
			...
			// This function aggreagate the ts vector by unit of second
			std::map<uint64_t, uint64_t> TimeAggregate(std::vector<uint64_t>& ts) {
				std::map<uint64_t, uint64_t> ta;
				for(auto& t : ts) {
					uint64_t sec = t / 1e6;
					auto it = ta.find(sec);
					if(it == ta.end())
			...
			void MixGraph(ThreadState* thread) {
				...
				// timestamp
				int tid = thread->tid;
				std::vector<uint64_t> get_ts;
				std::vector<uint64_t> put_ts;
				std::vector<uint64_t> seek_ts;
				std::map<uint64_t, uint64_t> get_agg;
				std::map<uint64_t, uint64_t> put_agg;
				std::map<uint64_t, uint64_t> seek_agg;
				uint64_t tp_start = FLAGS_env->NowMicros();
				uint64_t tp_cur;
				...
				while (!duration.Done(1)) {
					...
					if (usecs_since_last >
						...
						//write_rate = mix_rate_with_noise * query.ratio_[1] * FLAGS_mix_ave_kv_size;
					...
					if (query_type == 0) {
					  ...
					  //	read % 256 == 255) {
						...
						thread->stats.FinishedOps(db_with_cfh, db_with_cfh->db, 1, kRead);
						tp_cur = FLAGS_env->NowMicros();
						get_ts.push_back(tp_cur - tp_start);
						...
						tp_cur = FLAGS_env->NowMicros();
						put_ts.push_back(tp_cur - tp_start);
						...
						tp_cur = FLAGS_env->NowMicros();
						seek_ts.push_back(tp_cur - tp_start);
					}
				}
				char msg[256];
				snprintf(msg, sizeof(msg),
				...
				get_agg = TimeAggregate(get_ts);
				put_agg = TimeAggregate(put_ts);
				seek_agg = TimeAggregate(seek_ts);

				get_history_[tid] = get_agg;
				put_history_[tid] = put_agg;
				seek_history_[tid] = seek_agg;
				
				/* SUCCESS
				if(tid == 0) {
					std::string file = "/home/zzunny/KVstores/pmem-rocksdb/scripts/log-tid0";
					std::ofstream stream(file); //To Write into a File, Use "ofstream"
					for(auto& kv : get_agg) {
						stream << kv.first << "\t" << kv.second << '\n';
					}
					stream.close();
				...
				*/
				...
				if(tid == 0) {
					...
					for(auto& ts : get_ts) {
						uint64_t sec = ts / 1e6;
						stream << sec << "\n";
						auto it = get_agg.find(sec);
						if(it != get_agg.end()) it->second++;
						else get_agg.insert({sec, 1});
						//stream << kv.first << "\t" << kv.second << '\n';
					}
					for(auto& kv : get_agg) {
						stream << kv.first << "\t" << kv.second << '\n';
